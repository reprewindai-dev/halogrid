...
interface Props { region: Region | null; onClose: () => void }
    ...
    <AnimatePresence>
      {r && (
        <motion.div initial={{opacity:0,scale:0.94,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.94,y:8}}
          ...
          style={{ background:'rgba(6,13,24,0.96)', border:`1px solid ${stateColor(r.state)}25`,
          ...
          <div className="px-4 py-3 flex items-start justify-between"
            ...
            <button onClick={onClose} className="w-6 h-6 rounded-lg flex items-center justify-center transition-all"
          ...
          <div className="px-4 py-3 grid grid-cols-2 gap-3">
            {[
              { label:'CARBON',     value:`${r.carbon} gCO₂/kWh`,  color:stateColor(r.state) },
              ...
              <div key={m.label} className="rounded-xl px-3 py-2" style={{ background:'rgba(255,255,255,0.03)' }}>
                ...
                <div className="text-sm font-bold font-mono tabular-nums" style={{color:m.color}}>{m.value}</div>
              </div>
            ))}
          </div>
          <div className="px-4 pb-3">
            <div className="rounded-xl px-3 py-2.5 flex items-center justify-between"
              ...
              <div className="flex items-center gap-2">
                <span className="text-[9px] font-mono tracking-widest" style={{color:actionColor(r.lastDecision)}}>LAST ACTION</span>
